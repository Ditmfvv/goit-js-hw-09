const images = [
  ...
  // Генерація всіх карток зображень
  const imageCardsTemplate = images.map(createImageCardTemplate).join('');
  const imageListEl = document.querySelector('.gallery');
  
  // Додавання карток зображень в галерею
  imageListEl.innerHTML = imageCardsTemplate;
